...
function getOptions {
  while getopts ":a:e:k:r:i:b:c:p:S:K:s:h" opt;
  do
   case "${opt}" in
     a) APP_NAME=${OPTARG}
...
if [ -n "${APPDYNAMICS_SIM_ENABLED:+1}" ]; then
    sed -i.bak -e "s/#APPDYNAMICS_SIM_ENABLED:/APPDYNAMICS_SIM_ENABLED: $APPDYNAMICS_SIM_ENABLED/" ./.ebextensions/appd.config
fi
