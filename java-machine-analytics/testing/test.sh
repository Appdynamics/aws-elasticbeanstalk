...
function getOptions {
  while getopts ":a:e:k:r:i:b:c:p:S:K:s:g:E:h" opt;
  do
   case "${opt}" in
     a) APP_NAME=${OPTARG}
...
sed -i.bak "s/#APPDYNAMICS_AGENT_GLOBAL_ACCOUNT_NAME:/APPDYNAMICS_AGENT_GLOBAL_ACCOUNT_NAME: \"$APPDYNAMICS_AGENT_GLOBAL_ACCOUNT_NAME\"/" ./.ebextensions/appd.config
sed -i.bak "s/#APPDYNAMICS_ANALYTICS_EVENT_ENDPOINT:/APPDYNAMICS_ANALYTICS_EVENT_ENDPOINT: \"$APPDYNAMICS_ANALYTICS_EVENT_ENDPOINT\"/" ./.ebextensions/appd.config
...
if [ -n "${APPDYNAMICS_SIM_ENABLED:+1}" ]; then
    sed -i.bak "s/#APPDYNAMICS_SIM_ENABLED:/APPDYNAMICS_SIM_ENABLED: $APPDYNAMICS_SIM_ENABLED/" ./.ebextensions/appd.config
fi
